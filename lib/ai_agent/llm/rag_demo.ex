defmodule AiAgent.LLM.RAGDemo do
  ...
  def benchmark_rag_performance(user, test_questions \\ nil) do
    ...
    Enum.each(results, fn {question, {status, response_length, docs_used}, duration} ->
      ...
      IO.puts(
        "  #{status_icon} #{duration}ms | #{docs_used} docs | #{response_length} chars | #{String.slice(question, 0, 40)}..."
      )
    end)
  ...
  defp setup_demo_documents(user) do
    ...
    demo_docs = [
      %{
        content:
          "Hey! Just wanted to let you know that my kid Tommy plays baseball every Saturday at the local park. We're really excited about the season. He's on the Tigers team this year!",
        source: "john.smith@gmail.com",
        type: "email"
      },
      %{
        content:
          "I've been thinking about selling my AAPL shares. The stock has been performing really well lately, up about 15% this quarter. What do you think about the timing?",
      ...
      %{
        content:
          "Meeting scheduled for next Tuesday at 2 PM with the Johnson family to discuss their college savings plan. They have twin daughters starting high school.",
        source: "calendar_event",
        type: "calendar"
      },
      %{
        content:
          "Looking into 401k rollover options. My company was acquired and I need to move my retirement funds. Can we set up a meeting to discuss the best approach?",
        source: "david.chen@outlook.com",
        type: "email"
      },
      %{
        content:
          "Thanks for the great advice on diversifying my portfolio. I've decided to go with the balanced approach we discussed. My wife is also interested in meeting with you.",
      ...
      api_key ->
        ...
        messages = [
          %{
            role: "system",
            content:
              "You are a helpful financial advisor assistant. Answer the user's question based on your general knowledge."
          },
