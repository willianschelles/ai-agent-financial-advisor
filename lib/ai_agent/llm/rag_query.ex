defmodule AiAgent.LLM.RAGQuery do
  ...
  def ask(user, question, opts \\ %{}) when is_binary(question) do
    ...
    similarity_threshold = Map.get(opts, :similarity_threshold, 0.3)  # Lowered from 0.6 to 0.3
    temperature = Map.get(opts, :temperature, 0.7)
    system_prompt = Map.get(opts, :system_prompt)
    
    with {:ok, relevant_docs} <- retrieve_relevant_context(user, question, context_limit, similarity_threshold),
         {:ok, llm_response} <- query_llm_with_context(question, relevant_docs, model, max_tokens, temperature, system_prompt) do
  ...
  def retrieve_relevant_context(user, question, limit \\ 5, threshold \\ 0.3) do
    ...
    case VectorStore.find_similar_documents(user, question, search_opts) do
      {:ok, documents} ->
        ...
        enhanced_docs = Enum.map(documents, fn doc ->
  ...
  - {:error, reason} on failure
  """
  def query_llm_with_context(question, relevant_docs, model, max_tokens, temperature, custom_system_prompt \\ nil) do
  ...
  def quick_ask(user, question, opts \\ %{}) do
    case ask(user, question, opts) do
      ...
      {:error, reason} -> "I couldn't find relevant information to answer that question. Error: #{reason}"
    end
  end
